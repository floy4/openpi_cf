...
class Attention(nn.Module):
    ...
    def __call__(self, xs, positions, attn_mask, kv_cache, return_last_query_attn=False):  # noqa: FBT002
        ...
        # Always return last_query_attn with consistent structure for scan compatibility
        # When return_last_query_attn=False, return a placeholder array with zeros
        batch_size = q.shape[0]
        num_query_heads = self.configs[0].num_heads
        seq_len = k.shape[1]
        if return_last_query_attn:
            # [B, K, G, T, S] -> select last query token (T-1), keep all heads.
            last_query_attn = einops.rearrange(probs[:, :, :, -1, :], "B K G S -> B (K G) S")
        else:
            # Placeholder array with zeros to maintain consistent pytree structure
            last_query_attn = jnp.zeros((batch_size, num_query_heads, seq_len), dtype=dtype)
...
class Block(nn.Module):
    ...
    def __call__(
        ...
        return_last_query_attn=False,  # noqa: FBT002
    ):
        ...
        post_attn, kv_cache, last_query_attn = attn(
            pre_attn,
            positions,
            attn_mask,
            kv_cache,
            return_last_query_attn=return_last_query_attn,
        )
        ...
        return xs, (kv_cache, last_query_attn)
...
class Module(nn.Module):
    ...
    def setup(self):
        ...
        block_cls = nn.remat(
            Block,
            prevent_cse=False,
            static_argnums=(6, 7),  # 0=self, 6=deterministic, 7=return_last_query_attn
    ...
    def __call__(
        ...
        *,
        ...
        embedded, (kv_cache, _) = self.layers(
            embedded,
            kv_cache,
            positions,
            mask,
    ...
    @at.typecheck
    def decode_with_last_query_attention(
        self,
        embedded: Sequence[at.Float[at.Array, "b _t _d"] | None],
        positions: at.Int[at.Array, "b t"],
        mask: at.Bool[at.Array, "b t s"],
        ...
        *,
        kv_cache: KVCache | None = None,
        deterministic: bool = True,
    ) -> tuple[Sequence[at.Float[at.Array, "b _t _d"] | None], KVCache, at.Float[at.Array, "b n s"]]:
        """Decode and return the last-query attention of a specific layer.

        The returned attention is post-softmax attention probabilities over keys,
        for the final query token of the current decode call, across all heads.
        """
        if not 0 <= layer_index < self.configs[0].depth:
            raise ValueError(f"layer_index must be in [0, {self.configs[0].depth - 1}], got {layer_index}")
        ...
        embedded, (kv_cache, all_last_query_attn) = self.layers(
            embedded,
            kv_cache,
            positions,
            mask,
        ...
        if all_last_query_attn is None:
            raise ValueError("Expected attention outputs when return_last_query_attn=True.")

        layer_last_query_attn = all_last_query_attn[layer_index]

        assert all(e.dtype == jnp.dtype(self.embed_dtype) for e in embedded if e is not None)

        return [
            f(e, a)[0] if e is not None else e for f, e, a in zip(self.final_norms, embedded, adarms_cond, strict=True)
        ], kv_cache, layer_last_query_attn
